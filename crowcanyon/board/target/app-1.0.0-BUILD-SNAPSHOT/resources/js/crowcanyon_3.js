...
$(window).scroll(function(){
    ...
    let content_L_bot = $('.content_L').offset().top + $('.content_L').height() //비교되는 값 고정 (높이가 원래 정해져 있다면 >  지금은 나중에 이미지 넣어주니까 변하는 값으로)
    let target_bot = s_top + h_height + $('.content_R').outerHeight(); // 계속 변하는 값

    if(content_L_bot >= target_bot) {
        $('.content_R').css({
            position: 'fixed',
            // bottom : '133px'
            bottom: 'auto'
    ...
    else if(content_L_bot < target_bot) {
        $('.content_R').css({
            position: 'absolute',
            bottom: 0
        })
