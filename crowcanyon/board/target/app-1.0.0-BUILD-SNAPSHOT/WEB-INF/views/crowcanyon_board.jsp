...
<script>
    ...
    for(let i=0; i<sp.length; i++) {
        if(navigator.userAgent.toLowerCase().match(sp[i])) {
            location.replace("<c:url value='/m.board/write'/>")
        }
    }
</script>
<body>
...
<script>
    $(document).ready(function() {
        var listUrl = "${pageContext.request.contextPath}/board/list?page=${page}&pageSize=${pageSize}";
        var writeUrl = "${pageContext.request.contextPath}/board/write";
        var removeUrl = "${pageContext.request.contextPath}/board/remove";
