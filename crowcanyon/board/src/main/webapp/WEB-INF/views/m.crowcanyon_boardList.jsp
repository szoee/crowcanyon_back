    ...
    <div class="content">
        ...
        <table>
            ...
            <tr>
                <td>${boardDto.bno}</td>
                <td>
                    <a href="<c:url value="/m.board/read?bno=${boardDto.bno}&page=${ph.page}&pageSize=${ph.pageSize}" />">
                        ${boardDto.title}
                    </a>
                </td>
        ...
        <div class="pagination">
            <c:if test="${ph.showPrevBtn}">
                <a href="<c:url value="/m.board/list?page=${ph.startPage - 1}&pageSize=${ph.pageSize}" />" class="startPage">[PREV]</a>
            </c:if>

            <c:forEach var="i" begin="${ph.startPage}" end="${ph.endPage}">
                <a href="<c:url value="/m.board/list?page=${i}&pageSize=${ph.pageSize}" />" class="${i==ph.page?"pageActive":""}">${i}</a>
            </c:forEach>

            <c:if test="${ph.showNextBtn}">
                <a href="<c:url value="/m.board/list?page=${ph.endPage + 1}&pageSize=${ph.pageSize}" />" class="endPage">[NEXT]</a>
            </c:if>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        $('#btn_write').on('click',function(){
            location.href="<c:url value="/m.board/write"/>";
        });
